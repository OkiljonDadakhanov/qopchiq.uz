...
const calculateBMI = asyncHandler(async (req, res) => {
  const { userId, height, weight, age, gender, activityLevel, notes } = req.body
...
const getBMIHistory = asyncHandler(async (req, res) => {
  ...
  const bmiHistory = await HealthMetric.getBMIHistory(userId, Number.parseInt(limit))
...
const getHealthMetrics = asyncHandler(async (req, res) => {
  ...
  const healthScore = calculateHealthScore(latestMetrics, waterIntake, weeklyExercise)

  // Generate recommendations
  const recommendations = generateHealthRecommendations(latestMetrics, waterIntake, weeklyExercise)
...
const logExercise = asyncHandler(async (req, res) => {
  const { userId, name, type, duration, caloriesBurned, intensity, notes } = req.body
  ...
  sendResponse(res, 201, { exercise, coinsEarned }, "Exercise logged successfully")
})
...
const getExerciseHistory = asyncHandler(async (req, res) => {
  ...
  const exercises = await Exercise.find(query).sort({ date: -1 }).skip(skip).limit(Number.parseInt(limit))
...
const generateHealthRecommendations = (healthMetric, waterIntake = null, weeklyExercise = null) => {
  ...
  switch (bmiCategory) {
    ...
    case "obese":
      recommendations.push({
        ...
        action: "Consult with healthcare professionals for a comprehensive plan",
      })
      break
    default:
      recommendations.push({
  ...
  if (weeklyExercise) {
    const totalDuration = weeklyExercise.reduce((sum, ex) => sum + ex.totalDuration, 0)
...
const calculateHealthScore = (healthMetric, waterIntake, weeklyExercise) => {
  ...
  const waterScore = Math.min(30, Math.floor((waterIntake.totalAmount / 2000) * 30))
  score += waterScore
