...
const getOverview = asyncHandler(async (req, res) => {
  ...
  const [expenseStats, mealStats, healthStats, gamificationStats, trendsData, insights] = await Promise.all([
...
const getExpenseAnalytics = asyncHandler(async (req, res) => {
  ...
  // Get budget analysis
  const user = await User.findByTelegramId(userId)
  const budgetAnalysis = analyzeBudget(currentData.totalAmount, user.monthlyLimit, period)
  ...
  sendResponse(res, 200, {
    ...
    spendingPatterns,
    budgetAnalysis,
    recommendations: generateExpenseRecommendations(currentData, budgetAnalysis),
  })
})
...
const getMealAnalytics = asyncHandler(async (req, res) => {
  ...
  const calorieAnalysis = analyzeCalorieIntake(currentData.avgDailyCalories, calorieGoal)
...
const getHealthAnalytics = asyncHandler(async (req, res) => {
  ...
  const healthTrends = calculateHealthTrends(healthHistory, waterData, exerciseData)

  // Get latest health score
  const latestHealth = healthHistory[0]
  const healthScore = latestHealth ? calculateHealthScore(latestHealth, waterData, exerciseData) : null
  ...
  sendResponse(res, 200, {
    ...
    waterIntake: {
      dailyData: waterData,
      avgDaily: waterData.length > 0 ? waterData.reduce((sum, day) => sum + day.totalAmount, 0) / waterData.length : 0,
      totalDays: waterData.length,
    },
    exercise: {
      byType: exerciseData,
      totalDuration: exerciseData.reduce((sum, ex) => sum + ex.totalDuration, 0),
      totalCalories: exerciseData.reduce((sum, ex) => sum + ex.totalCalories, 0),
      totalSessions: exerciseData.reduce((sum, ex) => sum + ex.sessionCount, 0),
    },
    trends: healthTrends,
    healthScore,
    recommendations: generateHealthRecommendations(latestHealth, waterData, exerciseData),
  })
})
...
const getHealthStats = async (userId, dateRange) => {
  ...
  return {
    ...
    weeklyExercise: weeklyExercise.reduce((sum, ex) => sum + ex.totalDuration, 0),
    healthScore: latestHealth ? calculateHealthScore(latestHealth, waterToday, weeklyExercise) : null,
  }
}
...
const getGamificationStats = async (userId, dateRange) => {
  ...
  return {
    ...
    coinsEarned: achievements.reduce((sum, achievement) => sum + achievement.coinsRewarded, 0),
  }
}
...
const generateInsights = async (userId, dateRange) => {
  ...
  if (budgetUsage > 90) {
    ...
    insights.push({
      ...
      message: `You're doing well! Only ${Math.round(budgetUsage)}% of budget used`,
      priority: "low",
    })
  }
  ...
  if (topCategory && categorySpending[topCategory] > totalSpent * 0.4) {
    insights.push({
      ...
      message: `${Math.round((categorySpending[topCategory] / totalSpent) * 100)}% of your spending is on ${topCategory}`,
      priority: "medium",
    })
  }
...
const getDetailedMealAnalytics = async (query) => {
  ...
  const days = Math.ceil((query.date.$lte - query.date.$gte) / (1000 * 60 * 60 * 24))
...
const calculateHealthScore = (healthMetric, waterIntake, weeklyExercise) => {
  ...
  const totalExercise = weeklyExercise.reduce((sum, ex) => sum + ex.totalDuration, 0)
...
const generateExpenseRecommendations = (data, budgetAnalysis) => {
  ...
  if (budgetAnalysis.usage > 80) {
    recommendations.push("Consider reducing spending in your top category to stay within budget")
  }

  if (data.avgAmount > 50000) {
    recommendations.push("Try to break down large expenses into smaller, more manageable amounts")
  }
...
const generateMealRecommendations = (data, calorieAnalysis) => {
  ...
  if (data.avgDailyCalories < 1500) {
    recommendations.push("Consider increasing your daily calorie intake for better health")
...
const generateHealthRecommendations = (healthMetric, waterData, exerciseData) => {
  const recommendations = []

  if (healthMetric && healthMetric.bmiCategory !== "normal") {
    recommendations.push("Consider consulting with a healthcare professional about your BMI")
  }

  const avgWater =
    waterData.length > 0 ? waterData.reduce((sum, day) => sum + day.totalAmount, 0) / waterData.length : 0
  if (avgWater < 2000) {
    recommendations.push("Increase your daily water intake to at least 2000ml")
  }

  const totalExercise = exerciseData.reduce((sum, ex) => sum + ex.totalDuration, 0)
...
const calculateHealthTrends = (healthHistory, waterData, exerciseData) => {
  ...
  if (waterData.length >= 7) {
    const recent = waterData.slice(-7).reduce((sum, day) => sum + day.totalAmount, 0) / 7
    const earlier = waterData.slice(0, 7).reduce((sum, day) => sum + day.totalAmount, 0) / 7
    const change = recent - earlier
    trends.water = {
      current: Math.round(recent),
      change: Math.round(change),
      direction: change > 100 ? "increasing" : change < -100 ? "decreasing" : "stable",
    }
  }
