...
const createUser = asyncHandler(async (req, res) => {
  const { telegramId, username, firstName, lastName, language, monthlyLimit, preferences } = req.body

  // Check if user already exists
  const existingUser = await User.findOne({ telegramId })
  if (existingUser) {
    return sendError(res, 400, "User with this Telegram ID already exists")
  }
...
// @route   DELETE /api/users/:telegramId
// @access  Public
...
// @route   GET /api/users/:telegramId/stats
// @access  Public
const getUserStats = asyncHandler(async (req, res) => {
  ...
  const averageCoinsPerDay = daysSinceJoined > 0 ? Math.round(user.coins / daysSinceJoined) : 0
  ...
  const stats = {
    ...
    balancePercentage: user.monthlyLimit > 0 ? Math.round((user.currentBalance / user.monthlyLimit) * 100) : 0,
  }
...
// @route   POST /api/users/:telegramId/coins
// @access  Public
...
// @route   POST /api/users/:telegramId/badges
// @access  Public
...
// @route   POST /api/users/:telegramId/streak
// @access  Public
