...
const createMeal = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Database not connected, create mock meal
    if (userId === "507f1f77bcf86cd799439011") {
      meal = {
...
const getMealAnalytics = asyncHandler(async (req, res) => {
  ...
  const totalCalories = mealTypeAnalytics.reduce(
  ...
  sendResponse(res, 200, {
    ...
    totalCalories,
    averageDaily:
      dailyCalories.length > 0 ? totalCalories / dailyCalories.length : 0,
...
const getRecentMeals = asyncHandler(async (req, res) => {
  ...
  const meals = await Meal.find({ userId, isDeleted: false })
    .sort({ date: -1 })
    .limit(Number.parseInt(limit));
