...
const getExpenses = asyncHandler(async (req, res) => {
  ...
  const expenses = await Expense.find(query).sort(sort).skip(skip).limit(Number.parseInt(limit))
...
const createExpense = asyncHandler(async (req, res) => {
  const { userId, amount, category, description, emoji, date, mood, location, currency, tags, paymentMethod } = req.body
...
const updateExpense = asyncHandler(async (req, res) => {
  ...
  if (updateData.amount && updateData.amount !== oldAmount) {
    const user = await User.findByTelegramId(expense.userId)
    if (user) {
...
const getExpenseAnalytics = asyncHandler(async (req, res) => {
  ...
  const totalAmount = categoryAnalytics.reduce((sum, cat) => sum + cat.totalAmount, 0)

  sendResponse(res, 200, {
    period,
    totalExpenses,
    totalAmount,
    averageDaily: dailySpending.length > 0 ? totalAmount / dailySpending.length : 0,
...
const getRecentExpenses = asyncHandler(async (req, res) => {
  ...
  const expenses = await Expense.find({ userId, isDeleted: false }).sort({ date: -1 }).limit(Number.parseInt(limit))

  sendResponse(res, 200, { expenses }, "Recent expenses retrieved successfully")
})
